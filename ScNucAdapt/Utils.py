...
#def eas(eas_true_list,unknown_mask_list):



def compute_eas_true(unknown_celltype_list,cell_types):
    eas_true_list = []
    if unknown_celltype_list == None:
        eas_true_list = np.zeros(len(cell_types))
    if unknown_celltype_list != None:
        for i in cell_types:
            if i in unknown_celltype_list:
                eas_true_list.append(1)
            if i not in unknown_celltype_list:
                eas_true_list.append(0)
    eas_true_list = np.array(eas_true_list)
    return eas_true_list

def eas(unknown_celltype_list,predicted,true):
    n_k = 0
    n_u = 0
    n_k_k = 0
    n_u_k = 0
    ...
    for i,j in zip(true,predicted):
        ...
        if i in unknown_celltype_list:
            ...
            n_u_save.append(j)
    for p in n_u_save:
        if p not in unknown_celltype_list:
            n_u_k += 1
    for o in n_k_save:
        if o not in unknown_celltype_list:
            n_k_k += 1

    return (n_k_k/n_k)-(n_u_k/n_u)


def select_known(known_cell_list,data,labels):
    list = []
    for i,j in zip(range(len(data)),labels):
        if j in known_cell_list:
            list.append(data[i])
